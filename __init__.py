...
def init(cbpi):
    ...
    def lcd(LCDaddress):
        try:
            ...
            cbpi.notify('LCD Address is wrong',
                        'Change LCD Address in parameters,to detect comand promt in Raspi: sudo i2cdetect -y 1',
    ...
    def set_parameter_refresh():
        ...
        if ref is None:
            cbpi.add_config_parameter('LCD_Refresh', 5, 'number',
                                      'Time to remain till next display in sec, CBPi reboot required')
            ref = cbpi.get_config_parameter('LCD_Refresh', None)
        return ref
    ...
    def set_parameter_multidisplay():
        ...
        if multi is None:
            cbpi.add_config_parameter('LCD_Multidisplay', 'on', 'select',
                                      'Toggle between all Kettles or show only one Kette constantly, CBPi reboot required',
                                      ['on', 'off'])
            multi = cbpi.get_config_parameter('LCD_Multidisplay', None)
        return multi
        ...
        if s is not None:
            ...
            if multidisplay == "on":
                ...
                if s.timer_end is not None:
                    time_remaining = time.strftime(u"%H:%M:%S", time.gmtime(s.timer_end - time.time()))
                    line2 = (
                    (u"%s %s" % ((cbpi.cache.get("kettle")[id1].name).ljust(12)[:11], time_remaining)).ljust(20)[:20])
                else:
                    line2 = ((u'%s' % (cbpi.cache.get("kettle")[id1].name)).ljust(20)[:20])

                line3 = (u"Targ. Temp:%6.2f%s" % (float(cbpi.cache.get("kettle")[id1].target_temp), (u"°C"))).ljust(20)[
                        :20]
